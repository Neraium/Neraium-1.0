...
def evaluate_temporal_math(
    *,
    ...
    def mark(step: str, completed: int) -> None:
        step_timings[step] = round(__import__("time").perf_counter() - started, 4)
        if progress_callback:
            try:
                progress_callback(step, completed, total_calculations, dict(step_timings))
            except Exception:
                pass

    if progress_callback:
        try:
            progress_callback("prepare_numeric_matrix", 0, total_calculations, dict(step_timings))
        except Exception:
            pass
