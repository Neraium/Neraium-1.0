...
- `mapping_confidence` uses only persisted canonical role availability and uniqueness. It is `high` when both relationship signals have unique canonical roles, `low` when another catalog signal competes for either role, and `unknown` when role evidence is unavailable. It has no numeric score.
