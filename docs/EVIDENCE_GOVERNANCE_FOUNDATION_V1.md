...
The entire retained graph must be available by `evaluated_at`, including upstream and otherwise unused nodes. Evaluation rejects later evidence creation/effective times and later source-window end times on evidence or observations. Equal timestamps are allowed and timezone offsets normalize to UTC. Observation references in v1 have no separate creation/effective fields; their supplied window bounds are checked without inventing missing metadata. Adapters remain responsible for accurate availability metadata and immutable external references.
...
Supersession also requires the successor's `decision_timestamp` to be at or after its predecessor's, checked within the atomic append transaction. Retroactive `effective_timestamp` remains representable and does not relax decision chronology. A decision's lifecycle basis must already be effective at `decision_timestamp`; future-effective lifecycle events remain valid records but cannot supply the current state early. Storage independently validates the graph against the maturity evaluation timestamp before replay, so an imported, backdated maturity record cannot pass merely because replay reproduces its claimed result.
