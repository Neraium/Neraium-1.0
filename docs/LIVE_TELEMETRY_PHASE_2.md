...
`evaluate_relationship_against_baseline` isolates the relationship-fit
thresholds and persistence-window evaluator shared with the historical
assessment path. Phase 2 includes that shared evaluator without adding the
historical assessment's routes, storage, or presentation workflow.
