...
Prior diagnostics remain [in history](history-before-human-authority/TESTS.md).

| Selection | Result | Evidence |
|---|---|---|
| Pre-gate focused backend selection | 54 passed | targeted-final.xml, targeted-final.log, targeted-nodes.txt |
| Added upload correlation / bridge checks | 2 passed (one overlapping check) | upload-runtime-tests.xml |
| Post-gate signal-order regression and rank checks | 4 passed | post-gate-signal-order-tests.xml |
| Final typed identity / rank / historical / projection selection | 25 passed | final-contract-tests.xml |
| Terminal publication, retry correlation, legacy completion and stale-result tests | 4 passed | terminal-runtime-tests.xml |
| Frontend retirement unit file | 5 passed | npm test -- --run src/components/SystemTopologyWorkspace.test.js |
| Frontend production build | Passed | npm run build |
| One complete 10K repeat gate | FAILED retained analytical comparison; current repeat equality passed | complete-10k-tests.xml, complete-10k.log, gate/ |

All Python runs used TARGET's existing .venv, PYTHONDONTWRITEBYTECODE=1, pytest -p no:cacheprovider and isolated runtime storage. JUnit records exact test names. Frontend used installed locked dependencies; no browser/E2E test ran, so browser setup was not invoked. No full suite, broad regression campaign, 500K, 1M or LBNL execution.

Initial failed diagnostics are retained: execution correlation leaked into raw evidence hashes/metadata; those producer paths were corrected. The initial historical replay test used an incomplete canonical fixture; the final test verifies actual artifact decode without invoking the current builder. Optimization #5 initially differed only in build_commit; the test now explicitly validates SOURCE's recorded commit and TARGET's actual commit while comparing all telemetry, alias and hash fields unchanged. A first post-gate edit missed the second sorting expression; the regression caught it and the corrected two-site edit passed. These are not hidden or counted as successful runs.

The final small test results supersede failed diagnostic attempts, but no small selection supersedes the FAILED complete gate. Its comparator remains frozen. The corrected final candidate requires a newly authorized complete gate.

Static checks: git diff reviewed; git diff --check in both repositories; Python AST parsing; SOURCE snapshot hashes unchanged; three untouched TARGET-only files verified byte-for-byte; runner top-level functions match #4-absent SOURCE; protected consequence/graph/baseline/replay/store/package files match SOURCE. Full delta and exact function/hunk mapping are in promotion.patch and production-hunks.json.

Distinct backend tests with passing retained results across these focused selections: **62**. Repeated checks are counted once; results apply to the code versions stated above.
