...
3. Analysis upload processing first runs through `backend/app/services/historical_ingestion.py`, which preserves the raw source, profiles timestamp/signal/unit/quality evidence, and emits the versioned canonical handoff described in `docs/HISTORICAL_DATA_INGESTION_TRUST_V1.md`.
4. `backend/app/services/upload_jobs.py` passes only included canonical values and their trust provenance into `backend/app/services/upload_pipeline.py`; baseline uploads use the same canonical handoff through the dedicated `backend/app/services/behavioral_baseline.py` orchestration. `upload_pipeline.py` calls `backend/app/engine/sii_engine.py::evaluate_sii` exactly once.
