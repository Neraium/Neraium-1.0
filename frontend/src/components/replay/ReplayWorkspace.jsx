      ...
      <Panel title="Infrastructure Diagnostics" className="span-12 workspace-hero-panel" subtitle="Technical replay, topology, and evidence overlays separated from the operator flow.">
        ...
        {expertMode ? (
          <div className="structural-replay-controls">
            <button type="button" className="btn btn--secondary" onClick={() => setIsPlaying((value) => !value)}>{isPlaying ? "Pause Replay" : "Play Replay"}</button>
            <button type="button" className="btn btn--secondary" onClick={() => setComparisonMode((value) => !value)}>{comparisonMode ? "Primary View" : "Comparison Mode"}</button>
            <button type="button" className="btn btn--secondary" onClick={() => setFrameIndex((value) => Math.max(value - 1, 0))}>Previous Frame</button>
            <button type="button" className="btn btn--secondary" onClick={() => setFrameIndex((value) => Math.min(value + 1, operativeTimeline.length - 1))}>Next Frame</button>
            <label className="metadata-text" htmlFor="replay-speed">Speed</label>
            <select id="replay-speed" value={playbackSpeed} onChange={(event) => setPlaybackSpeed(Number(event.target.value))}>{[0.5, 1, 1.5, 2, 4].map((speed) => <option key={speed} value={speed}>{speed}x</option>)}</select>
            ...
            <input type="range" min={0} max={Math.max(0, operativeTimeline.length - 1)} value={Math.min(frameIndex, Math.max(0, operativeTimeline.length - 1))} onChange={(event) => setFrameIndex(Number(event.target.value))} />
          </div>
        ) : (
          <div className="structural-replay-controls">
        ...
        <p className="metadata-text">Diagnostic timestamp: {shownFrame?.timestamp ? formatClockTime(shownFrame.timestamp) : "model-generated continuity frame"}</p>
        <ReplayCognitionField timeline={operativeTimeline} frameIndex={Math.min(frameIndex, Math.max(0, operativeTimeline.length - 1))} isPlaying={isPlaying} comparisonMode={comparisonMode} formatClockTime={formatClockTime} />
      </Panel>
      {expertMode ? (
        <Panel title="State-Space Progression" className="span-12 replay-phase-panel">
          <div className="canonical-flow">
            {canonicalFlow.map((phase) => <div key={phase} className={`canonical-flow__step ${shownFrame?.cognition_state?.canonical_phase === phase ? "is-active" : ""}`}><span>{phase.replaceAll("_", " ")}</span></div>)}
          </div>
        </Panel>
      ) : null}
...
function buildCultivationReplayDemo() {
  const now = Date.now();
  const timeline = [
    { timestamp: new Date(now - 1000 * 60 * 60 * 6).toISOString(), topology_state: { stability_state: "STABLE", fragmentation_indicator: "low", phase: "stable_topology" }, propagation_state: { propagation_acceleration: "low", dominant_paths: ["airflow_balance"], recovery_convergence: "stable" }, cognition_state: { canonical_phase: "stable_topology", confidence_tier: "HIGH_EVIDENCE", operational_phase: "monitoring", facility_state: "Monitoring" }, continuation_window: { window: "14 operational days", timing_window: "low urgency" }, subsystem_pressure: { compression_intensity: "low" }, evidence_state: { lineage_events: [{ target: "airflow_balance", evidence_sources: { topology_evidence: ["Room synchronization stable"] } }] } },
    { timestamp: new Date(now - 1000 * 60 * 60 * 4).toISOString(), topology_state: { stability_state: "WATCH", fragmentation_indicator: "moderate", phase: "relationship_weakening" }, propagation_state: { propagation_acceleration: "moderate", dominant_paths: ["airflow_imbalance_to_thermal_lag"], recovery_convergence: "monitoring" }, cognition_state: { canonical_phase: "propagation_activation", confidence_tier: "MODERATE_EVIDENCE", operational_phase: "drift_review", facility_state: "Drift observed" }, continuation_window: { window: "7 to 12 operational days", timing_window: "compression emerging" }, subsystem_pressure: { compression_intensity: "moderate" }, evidence_state: { lineage_events: [{ target: "compensation_masking", evidence_sources: { propagation_evidence: ["HVAC response lag", "Humidity compensation rise"] } }] } },
    { timestamp: new Date(now - 1000 * 60 * 60 * 2).toISOString(), topology_state: { stability_state: "DETERIORATING", fragmentation_indicator: "high", phase: "structural_fragmentation" }, propagation_state: { propagation_acceleration: "high", dominant_paths: ["airflow_imbalance_to_thermal_lag_to_vpd_drift"], recovery_convergence: "delayed" }, cognition_state: { canonical_phase: "continuation_pathways", confidence_tier: "HIGH_EVIDENCE", operational_phase: "operator_review", facility_state: "Escalation watch" }, continuation_window: { window: "3 to 6 operational days", timing_window: "elevated urgency" }, subsystem_pressure: { compression_intensity: "high" }, evidence_state: { lineage_events: [{ target: "vpd_decoupling", evidence_sources: { persistence_evidence: ["VPD coupling drift persisted across replay frames"] } }] } },
  ];
  return { timeline, meta: { frame_count: timeline.length, intervals: timeline.length, replay_compression: 1, canonical_flow: DEFAULT_CANONICAL_FLOW } };
}
