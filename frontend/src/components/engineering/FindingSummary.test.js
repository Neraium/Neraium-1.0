...
describe("compact Results finding card", () => {
  ...
  it("omits optional state, assignment, and limitation without empty panels", () => {
    render(React.createElement(FindingSummary, { card: compactCard({ reviewState: null, assignment: null, materialLimitation: null }) }));
    const card = screen.getByTestId("compact-finding-card");
    expect(within(card).queryByText("Review state")).toBeNull();
    expect(within(card).queryByText("Assignment")).toBeNull();
