...
function SiteOverview({ projection, onReview }) {
  return <OperationsBrief projection={projection} onReview={onReview} />;
}

function SystemOverview({ system, systemsProjection, onReview, onSystem }) {
  if (!system) return <SystemsOverview projection={systemsProjection} onSystem={onSystem} />;
  return <OperationsBrief projection={system.results} onReview={onReview} />;
}
              ...
              : effectiveRoute === "trace" ? <TraceWorkspace model={model} finding={selectedFinding} apiFetch={apiFetch} onBack={() => goBack("investigations")} />
                      : effectiveRoute === "portfolio" ? <PortfolioWorkspace sites={portfolioSites} onSelectSite={handleSelectSite} />
                        : effectiveRoute === "systems" ? <SystemsOverview projection={systemsProjection} onSystem={openSystem} />
                          : effectiveRoute === "findings" ? <FindingsOverview projection={resultsProjection} onReview={openFinding} />
                            : effectiveRoute === "investigations" ? <EvidenceOutcomesOverview projection={resultsProjection} onReview={openFinding} />
                            : effectiveRoute === "system" ? <SystemOverview system={selectedSystem} systemsProjection={systemsProjection} onReview={openFinding} onSystem={openSystem} />
                              : presentationState.key === "legacyAnalysis" ? <WorkspaceStateNotice state={presentationState} onPrimary={presentationPrimaryAction} />
                                : <SiteOverview projection={resultsProjection} onReview={openFinding} />}
        </main>
      </div>
    </div>
  );
