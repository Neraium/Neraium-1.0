...
function chipsForFrame(frame) {
  ...
  const paths = (frame?.propagation_state?.dominant_paths ?? []).map((path, index) => ({
    id: `path-${index}`,
    label: String(path).replaceAll("_", " "),
    type: "Propagation Path",
    payload: { path },
  }));
  const continuation = [{
    id: "continuation",
    label: frame?.continuation_window?.window ?? "Continuation window",
    type: "Continuation Window",
    payload: frame?.continuation_window ?? {},
  }];
  const pressure = [{
    id: "pressure",
    label: frame?.subsystem_pressure?.compression_intensity ?? "Compression",
    type: "Subsystem Pressure",
    payload: frame?.subsystem_pressure ?? {},
  }];
  const cognition = [{
    id: "cognition",
    label: frame?.cognition_state?.confidence_tier ?? "Cognition State",
