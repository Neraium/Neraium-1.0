...
function assignmentLabel(reviewRecord) {
  ...
  return firstText(own(assignment, "label"), own(assignment, "name"), own(reviewRecord, "assignedTo"), own(reviewRecord, "owner")) || null;
}
...
export function projectResults(model, reviewRecords = {}, options = {}) {
  ...
  if (status === "Evidence insufficient") {
    ...
    return {
      ...
      auditAction: scoped ? { label: "Open evidence record", route: route("evidence", own(scoped, "id")) } : null,
    };
  }
