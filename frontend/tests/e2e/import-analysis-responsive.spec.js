...
test("active analysis remains readable and contained on narrow mobile", async ({ page }) => {
  ...
  await expect(page.locator(".upload-processing-status")).toContainText("Analysis active - last update just now");
  ...
  const metrics = await page.evaluate(() => {
    ...
    const chip = document.querySelector(".upload-file-chip")?.getBoundingClientRect();
